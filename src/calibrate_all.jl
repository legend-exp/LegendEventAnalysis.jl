...
const _VALID_CALIBRATE_ALL_SUBSYSTEMS = (:geds, :spms, :pmts, :aux)

# per-detector data from an open LH5 store (tier/<det>) or from a read_ldata NamedTuple
_detdata(ds::NamedTuple, detector, tier) = ds[Symbol(detector)]
_detdata(ds, detector, tier) = ds[tier, string(detector)][:]
_hasdet(ds::NamedTuple, detector, tier) = haskey(ds, Symbol(detector))
_hasdet(ds, detector, tier) = haskey(ds, "$tier/$detector")

"""
    calibrate_all(data::LegendData, sel::ValiditySelection, datastore::AbstractDict; subsystems::Vector{Symbol}=[:geds, :spms, :pmts, :aux])

Calibrate all detectors in the given datastore, using the metadata
processing configuration for `data` and `sel`.

# Keyword arguments
- `subsystems::Vector{Symbol}=[:geds, :spms, :pmts, :aux]`: which subsystems
  to include. Any subset of `:geds` (HPGe), `:spms` (SiPM veto), `:pmts`
  (PMT veto) and `:aux` (auxiliary/FC channels) can be picked — only the
  metadata and data needed for the selected subsystems is loaded and
  processed.

  Cross-system columns that depend on more than one subsystem are only
  added to the result table when *all* of the subsystems they need are
  selected:
  - `ged_spm` needs `:geds` and `:spms`
  - `ft_spm` needs `:spms`
  - `ged_pmt` needs `:geds` and `:pmts`

  At least one of `:geds`, `:spms` or `:aux` must be selected, since these
  are the subsystems the global (cross-system) event table is built from.
  `:pmts` is always processed on its own (its calibrated/muon-cut table is
  returned as the second return value regardless of `subsystems`), but only
  contributes the `ged_pmt` column to the merged result table when `:geds`
  is also selected.
"""
function calibrate_all(data::LegendData, sel::AnyValiditySelection, datastore::AbstractDataStore, tier::DataTierLike=:jldsp; subsystems::Vector{Symbol}=collect(_VALID_CALIBRATE_ALL_SUBSYSTEMS))
    ds = datastore

    if !issubset(subsystems, _VALID_CALIBRATE_ALL_SUBSYSTEMS)
        throw(ArgumentError("`subsystems` must be a subset of $(_VALID_CALIBRATE_ALL_SUBSYSTEMS), got $(subsystems)"))
    end

    @debug "Calibrating all detectors for `ValiditySelection` $(sel) in `DataTier` $(tier), subsystems: $(subsystems)"
    chinfo = channelinfo(data, sel)

    # HPGe:
    ged_events = nothing
    if :geds in subsystems
        ...
        Threads.@threads for i in eachindex(geds_detectors)
            let detector = geds_detectors[i], detdata = _detdata(ds, detector, tier)
    ...
    if :spms in subsystems
        ...
        Threads.@threads for i in eachindex(spms_detectors)
            let detector = spms_detectors[i], detdata = _detdata(ds, detector, tier)
        ...
        @debug "Skipping SiPM calibration (:spms not in subsystems)"
    end

    # PMT:
    pmt_events = Vector{NamedTuple{(:timestamp, ), Tuple{Unitful.Time{<:Real}, }}}()
    if :pmts in subsystems
        ...
        pmt_events = if all(.!_hasdet.(Ref(ds), pmts_detectors, Ref(tier)))
            ...
            Threads.@threads for i in eachindex(pmts_detectors)
                let detector = pmts_detectors[i], detdata = _detdata(ds, detector, tier)
        ...
        @debug "Skipping PMT calibration (:pmts not in subsystems)"
    end

    # Aux:
    aux_events = NamedTuple()
    if :aux in subsystems
        ...
        aux_caldata =
            [Dict(
                let detector = aux_detectors[i],
                    detdata = _detdata(ds, detector, tier)
        ...
        @debug "Skipping auxiliary calibration (:aux not in subsystems)"
    end
    ...
    if :geds in subsystems
        system_events = merge(system_events, (geds = ged_events,))
    end
    if :spms in subsystems
        system_events = merge(system_events, (spms = spm_events,))
    end
    system_events = merge(system_events, aux_events)

    if isempty(system_events)
        throw(ArgumentError("`subsystems` must include at least one of :geds, :spms or :aux to build the global event table"))
    end

    global_events_pre = build_cross_system_events(system_events)
    global_events = if :aux in subsystems
        ...
        StructVector(merge(Base.structdiff(columns(global_events_pre), NamedTuple{keys(aux_events)}), (aux = StructArray(aux_cols),)))
    else
        global_events_pre
    end

    cross_systems_cols = NamedTuple()
    if :geds in subsystems && :spms in subsystems
        cross_systems_cols = merge(cross_systems_cols, (ged_spm = _build_lar_cut(data, sel, global_events, global_events.geds.t0_start),))
    end
    if :spms in subsystems
        cross_systems_cols = merge(cross_systems_cols, (ft_spm = _build_lar_cut(data, sel, global_events, fill(get_spms_evt_lar_cut_props(data, sel).ft_cut_t0, length(global_events))),))
    end
    if :geds in subsystems && :pmts in subsystems
        cross_systems_cols = merge(cross_systems_cols, (ged_pmt = _build_muon_evt_cut(data, sel, global_events, pmt_events),))
    end
...
_fix_vov(x::VectorOfVectors{<:AbstractVector}) = VectorOfVectors(VectorOfVectors(flatview(x)), x.elem_ptr .- (first(x.elem_ptr) - 1))
